...
def aedat4_to_GTP(aedat_path, offset_path, out_dir, folder_name = 'imgs_GTP', width = 346,
                  height = 260, ch12_strength=40, ch3_strength=30, ch3_decay_rate=0.8, sub_div=1):
    ...
    for idx, event in enumerate(tqdm(events, leave=False, desc='Converting aedat4 to GTP')):
        ...
        if frame_idx < frame_start_num and event['timestamp'] >= frame_start_list[frame_idx]:  # 如果事件超过当前帧时间
            ...
            start_idx = idx
            cv2.imwrite(str(img_dir / f'frame_{((frame_idx - 1) * sub_div + sub_idx):06d}.png'), GTP_img)
            frame_idx += 1
...
if __name__ == '__main__':
    ...
    aedat4_to_GTP(Path(aedat_path), Path(offset),
                    out_dir, width=width, height=height, sub_div=1, ch3_decay_rate=0.7)
