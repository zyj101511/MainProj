...
from lib.train.trainer.base_trainer import BaseTrainer
...
class MASTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, use_amp=False):
        ...
        args:
            ...
            actor - The actor that handles the forward pass and loss calculation
            optimizer - The optimizer for training
            scheduler - The learning rate scheduler
            train_loader - DataLoader for training data
            val_loader - DataLoader for validation data
            settings - Training settings (e.g., number of epochs, device)
        """
        super().__init__(actor, loaders, optimizer, settings, lr_scheduler)
        self._set_default_settings()

        self.stats = {loader.name: None for loader in self.loaders}
    ...
    def cycle_dataset(self, loader):
        ...
        epoch_time = self.cur_time - self.start_time
        ...
        if self.stats[loader.name] is not None:
            for name, meter in self.stats[loader.name].items():
                if hasattr(meter, 'avg'):
                    print(f"Epoch Avg {name}: {meter.avg:.5f}")
    ...
    def _update_stats(self, new_stats, batch_size, loader):
        ...
        if loader.name not in self.stats.keys() or self.stats[loader.name] is None:
            self.stats[loader.name] = {name: AverageMeter() for name in new_stats.keys()}
    ...
    def _print_stats(self, i, loader, batch_size):
        ...
        batch_fps = batch_size / (current_time - self.cur_time)
        average_fps = self.num_frames / (current_time - self.start_time)
        prev_frame_time_backup = self.cur_time
