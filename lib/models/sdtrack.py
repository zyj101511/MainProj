...
from lib.models.heads.head import build_head
...
class SDTrack(nn.Module):
    def __init__(self, backbone, head, head_type='CENTER'):
        ...
        if head_type in ('CENTER', 'CORNER'):
            ...
            self.search_feature_size = int(head.search_feature_size)
    ...
    def forward(self, template: torch.Tensor, search: torch.Tensor):
        ...
        out = self._forward_head(feature=last_feature)
        # 记录辅助输出和backbone的输出特征
        out.update(aux_dict)
        out['backbone_feature'] = features
        return out

    def _forward_head(self, feature: torch.Tensor):
        ...
        if self.head_type == 'CORNER':
            ...
            pred_box, offset_map, size_map, score_map_ctr = self.head(feature=search_feature, return_dist=True)
            outputs_coord = pred_box
            out = {'pred_boxes': outputs_coord,
...
def _build_backbone_head(cfg):
    # 根据模型type创建backbone和head
    if cfg.MODEL.BACKBONE.TYPE == 'TINY':
        backbone = sdtrack_tiny()
        hidden_dim = backbone.embed_dim
        head = build_head(cfg, hidden_dim)
    else:
...
def build_model(cfg, training=True):
    ...
    backbone, head = _build_backbone_head(cfg)
