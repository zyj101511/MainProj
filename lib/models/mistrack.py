...
class MISTrack(nn.Module):
    ...
    def forward(self, search: torch.Tensor, template: torch.Tensor):
        ...
        fused_feature, _ = self._forward_multi_timescale_module(feature=features)
        ...
        # out_dict['branches_out_list'] = branches_out_list
        return out_dict
    ...
    def _forward_multi_timescale_module(self, feature: torch.Tensor):
        ...
        fused_feature, branches_out_list = self.multi_timescale_module(search_feature)  # (B, C, H, W)
        return fused_feature, branches_out_list
...
def _count_parameters(model):
    ...
    multi_timescale_params = sum(p.numel() for p in model.multi_timescale_module.parameters())
    track_head_params = sum(p.numel() for p in model.track_head.parameters())
    trajectory_head_params = sum(p.numel() for p in model.trajectory_head.parameters())
    return total_params, backbone_params, multi_timescale_params, track_head_params, trajectory_head_params
